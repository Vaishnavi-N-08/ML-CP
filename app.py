...
st.set_page_config(page_title="AutoML Tools",
...
with st.sidebar:
    ...
    st.image("https://www.onepointltd.com/wp-content/uploads/2020/03/inno2.png",
             caption='AutoML Tools')
    st.title("AutoML Tools")
    st.info("This project application helps explore your data and make forecasts.")
    profile_sample = pd.read_csv('./AB_NYC_2019.csv')
    profile_sample = profile_sample.to_csv().encode('utf-8')
    st.download_button(
        label="Download Sample for Profiling",
        data=profile_sample,
        file_name='profile_sample.csv',
        mime='text/csv',
    )
    forecast_sample = pd.read_csv('./sample_forecast.csv')
    forecast_sample = forecast_sample.to_csv().encode('utf-8')
    st.download_button(
        label="Download Sample for Forecasting",
        data=forecast_sample,
        file_name='forecast_sample.csv',
        mime='text/csv',
    )
    tabs = ["Profiling Tool", "Forecast Tool"]
...
code_options = ["Dataframe information", "Model fitting",
                "Cross validation", "Hyperparam tuning"]


if page == "Profiling Tool":
...
if page == "Forecast Tool":
    ...
    with st.container():
        ...
        with st.expander('Holidays'):
            ...
            countries = ['Country name', 'India', 'Italy', 'Spain',
            ...
            if output == 1:
                    ...
                    if st.button('Export forecast (.csv)'):
                        with st.spinner("Exporting.."):
                            ...
                            export_forecast = pd.DataFrame(
                                forecast[['ds', 'yhat_lower', 'yhat', 'yhat_upper']]).to_csv().encode('utf-8')
                            st.download_button(
                                label="Download CSV File",
                                data=export_forecast,
                ...
                with col2:
                    ...
                    if st.button("Export model metrics (.csv)"):
                        try:
                            df_p = df_p.to_csv(decimal=',').encode('utf-8')
                            st.download_button(
                                label="Download CSV File",
                                data=df_p,
                ...
                with col3:
                    if st.button('Save model configuration (.json) in memory'):
                        serialized_model = json.dumps(
                            model_to_json(m), ensure_ascii=False).encode('utf8')
                        st.download_button(
                            label="Download JSON File",
                            data=serialized_model,
                            file_name='serialized_model.json',
